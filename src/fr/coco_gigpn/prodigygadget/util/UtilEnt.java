...
public class UtilEnt implements Listener
{
	...
	private static HashMap<Player, ArrayList<Entity>> support = new HashMap<Player, ArrayList<Entity>>();
	private static HashMap<Player, ArrayList<Entity>> entity = new HashMap<Player, ArrayList<Entity>>();

	public static void spawnFlyingEntityToRemove(final Player p , Location l  ,Material m , byte data, int time) {
		support.put(p, new ArrayList<Entity>());
		entity.put(p, new ArrayList<Entity>());

		final WitherSkull skull = (WitherSkull) l.getWorld().spawn(
				l, WitherSkull.class);
		...
		((ArrayList<Entity>) support.get(p)).add(skull);
		((ArrayList<Entity>) entity.get(p)).add(ei.getBukkitEntity());

		Bukkit.getServer().getScheduler()
		.runTaskLater(Main.schedule, new Runnable() {
			public void run() {
				if (ei.getBukkitEntity().isValid()) {
					ei.getBukkitEntity().remove();
				}
				if (skull.isValid()) {
					skull.remove();
				}
				entity.get(p).remove(ei.getBukkitEntity());
				support.get(p).remove(skull);
	...
	public void skullDamage(EntityDamageEvent event) {
		...
		if (e instanceof WitherSkull || e instanceof Entity) {
			...
			if (support.containsValue(e) || entity.containsValue(e)) {
