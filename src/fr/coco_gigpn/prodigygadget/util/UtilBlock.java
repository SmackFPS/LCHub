...
public class UtilBlock
{
	...
	public static ArrayList<Block> blockToRestore = new ArrayList<Block>();
	...
	@SuppressWarnings("deprecation")
	public static void setBlock(Material m ,byte data, Location l) {
	...
	public static enum RiderType {
		...
		AngelEffect , GhostEffect , NyanEffect ,  PoseidonEffect , DarkRider

	}

	@SuppressWarnings({ "deprecation" })
	public static void setBlockToRestore(final Block block, int id, byte color, long timeInSecs, final RiderType effect) {
		blockToRestore.add(block);
		...
		Bukkit.getScheduler().runTaskLater(Main.schedule, new Runnable(){
			...
			@Override
			public void run() {
				float y = UtilMath.randomRangeFloat(0.2F,1.0F);
				...
				if (UtilLag.ServerisLag() && Configs.stopParticleOnLag == true) {
					...
					if(effect == RiderType.GhostEffect) {
						UtilParticle.sendParticleToLocation(l, Particle.FLAME ,UtilMath.randomRangeFloat(-1.0f,1.0f), y, UtilMath.randomRangeFloat(-1.0f,1.0f), UtilMath.randomRangeFloat(0.01f,0.1f), 10);

					}
					if(effect == RiderType.AngelEffect) {
						l.getWorld().playSound(l, Sound.STEP_SNOW, 1, 1);
						UtilParticle.sendParticleToLocation(l,Particle.SNOWBALL_POOF, UtilMath.randomRangeFloat(-1.0f,1.0f), y, UtilMath.randomRangeFloat(-1.0f,1.0f), UtilMath.randomRangeFloat(0.1f,0.5f), 10);
						UtilParticle.sendParticleToLocation(l, Particle.CLOUD, UtilMath.randomRangeFloat(-1.0f,1.0f), y, UtilMath.randomRangeFloat(-1.0f,1.0f), UtilMath.randomRangeFloat(0.01f,0.1f), 10);
						UtilParticle.sendParticleToLocation(l,Particle.SNOW_SHOVEL, UtilMath.randomRangeFloat(-1.0f,1.0f), y, UtilMath.randomRangeFloat(-1.0f,1.0f), UtilMath.randomRangeFloat(0.01f,0.1f), 10);
					}
					if(effect == RiderType.PoseidonEffect) {
						UtilParticle.sendParticleToLocation(l,Particle.DRIP_WATER, UtilMath.randomRangeFloat(-1.0f,1.0f), y, UtilMath.randomRangeFloat(-1.0f,1.0f), UtilMath.randomRangeFloat(0.1f,0.5f), 10);
						UtilParticle.sendParticleToLocation(l, Particle.MAGIC_CRIT, UtilMath.randomRangeFloat(-1.0f,1.0f), y, UtilMath.randomRangeFloat(-1.0f,1.0f), UtilMath.randomRangeFloat(0.01f,0.1f), 10);
					}
					if(effect == RiderType.DarkRider) {
						UtilParticle.sendParticleToLocation(l,Particle.LARGE_SMOKE, UtilMath.randomRangeFloat(-1.0f,1.0f), y, UtilMath.randomRangeFloat(-1.0f,1.0f), UtilMath.randomRangeFloat(0.1f,0.2f), 10);
						UtilParticle.sendParticleToLocation(l, Particle.WITCH_MAGIC, UtilMath.randomRangeFloat(-1.0f,1.0f), y, UtilMath.randomRangeFloat(-1.0f,1.0f), UtilMath.randomRangeFloat(0.01f,0.1f), 10);

					}
					if(effect == RiderType.NyanEffect) {

						UtilParticle.sendParticleToLocation(l.add(0 , 1 , 0),Particle.RED_DUST, UtilMath.randomRangeFloat(-1.0f,1.0f), y, UtilMath.randomRangeFloat(-1.0f,1.0f), 1, 10);	
					}

				}
				blockToRestore.remove(block);
