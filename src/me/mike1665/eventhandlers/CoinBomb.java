...
public class CoinBomb implements Listener {
	...
	public void launchBomb(Player p) {
	    Bukkit.broadcastMessage(StringManager.getPrefix(MessageType.TREASURE) + " §7§l" + p.getName() + " §a§ljust launched a GemBomb!");

	    ItemStack baseItem = new ItemStack(Material.AIR);
	    final ItemStack firedItem = new ItemStack(Material.AIR);
	    baseItem.setType(Material.EMERALD_BLOCK);
	    firedItem.setType(Material.DOUBLE_PLANT);
	        ...
	        if (this.running) {
	          if (this.times < 10) {
	            ...
	            if (!this.oposite)
	            {
	              ...
	              CoinBomb.this.firedEmys.add(emy1.getUniqueId());
	              CoinBomb.this.firedEmys.add(emy2.getUniqueId());
	              CoinBomb.this.firedEmys.add(emy3.getUniqueId());
	              CoinBomb.this.firedEmys.add(emy4.getUniqueId());

	              Firework fw = (Firework)base.getWorld().spawn(base.getLocation().subtract(0.0D, 2.0D, 0.0D), Firework.class);
	              FireworkEffect effect = FireworkEffect.builder().trail(true).flicker(false).withColor(Color.LIME, Color.GREEN).with(FireworkEffect.Type.BURST).build();
