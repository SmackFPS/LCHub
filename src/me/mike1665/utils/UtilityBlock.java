...
public class UtilityBlock
{
  ...
  public static ArrayList<Block> frostyBlock = new ArrayList();
  ...
  public static void setBlock(int id, byte data, Location l)
  {
    l.getBlock().setTypeId(id);
    l.getBlock().setData(data);
  }

  public static void setFakeBlock(int id, byte data, Location l)
  {
    int x = l.getBlockX();
    int y = l.getBlockY();
    int z = l.getBlockZ();
    for (Player p2 : UtilServer.getPlayers()) p2.sendBlockChange(l.getWorld().getBlockAt(x, y, z).getLocation(), id, data); 
  }
  ...
  public static ArrayList<Block> getSurrounding(Block block, boolean diagonals, boolean onlyHightest) {
      ...
      public void run()
      {
        if (playEffect) {
          for (Player p : UtilServer.getPlayers()) {
            UtilityBlock.sendBreak(p, id, data, block.getLocation().add(0.0D, 0.5D, 0.0D));
          }
        }
        if (fakeBlock)
          UtilityBlock.setFakeBlock(BeforeId, BeforeData, block.getLocation());
        else {
          UtilityBlock.setBlock(BeforeId, BeforeData, block.getLocation());
        }

        UtilityBlock.blockToRestore.remove(block);

        if (frosty)
          UtilityBlock.frostyBlock.remove(block);
      }
    }
    , timeInSecs * 20L);
  }
