...
public class PaintballGun implements Listener {
	...
	public void Paint(ProjectileHitEvent event) {
		...
		Location loc = event.getEntity().getLocation()
				.add(event.getEntity().getVelocity());
		...
		for (Block block : UtilBlock.getInRadius(loc, 1.5D).keySet()) {
			...
			if (UtilBlock.solid(block)) {
				...
				if (block.getType() == Material.CARPET) {
					setBlockToRestore(block, 35, color, 4L, (Entity) event
				...
				if (block.getType() != Material.WOOL) {
					...
					for (int k = 0; k < 7; k++)
						ParticleEffect.FIREWORKS_SPARK.display(bLoc, 0.0F,
								0.0F, 0.0F, 0.1F, 1);
				}
			}
		}
		...
		if (event.getAction() == Action.RIGHT_CLICK_AIR
			...
			if ((disName(p.getItemInHand()) != null)
				...
				player.getWorld().playSound(player.getLocation(),
						Sound.CHICKEN_EGG_POP, 1.5F, 1.5F);
			}
		}
	...
	@SuppressWarnings("deprecation")
	public void setBlockToRestore(final Block block, int id, byte color,
			long timeInSecs, Entity shooter) {
		...
		for (Player p2 : Bukkit.getOnlinePlayers())
			p2.sendBlockChange(block.getLocation(), id, color);
		Bukkit.getScheduler().runTaskLater(this.plugin, new Runnable() {
			public void run() {
				for (Player p2 : Bukkit.getOnlinePlayers())
					if (p != p2)
						p2.sendBlockChange(block.getLocation(), BeforeId,
								BeforeData);
			}
		}, timeInSecs * 5L);
	}
